...
2. **เมื่อคำถามเกี่ยวกับสิทธิสุขภาพ / การรักษาพยาบาล / กายอุปกรณ์ / สวัสดิการรัฐ**:
   - เชื่อมโยงสิทธิข้ามกระทรวงอย่างครอบคลุม: สปสช. (1330 - บัตรทอง 30 บาทรักษาทุกที่, UCEP เจ็บป่วยฉุกเฉินวิกฤตฟรี 72 ชม.), พม. (1300 - คนพิการ, เบี้ยยังชีพ, เตียงปรับระดับ, รถเข็น, เครื่องผลิตออกซิเจน), กปท./อบต./เทศบาล (ผ้าอ้อมผู้ใหญ่ฟรี <=3 ชิ้น/วัน, Caregiver), ประกันสังคม (1506 - ม.33/39/40, ทันตกรรม 900 บาท/ปี), กรมบัญชีกลาง (ข้าราชการ)
...
3. ตอบด้วยภาษาไทยที่สุภาพ กระชับ อ่านง่าย และจัดรูปแบบด้วย Markdown อย่างสวยงาม
"""
            ...
            if use_web_search and len(user_query.strip()) > 2:
                try:
                    ...
                    if web_results:
                        ...
                        web_text = "\n\n[ข้อมูลที่สืบค้นจากอินเทอร์เน็ตแบบเรียลไทม์ (Live Web Search)]:\n"
    ...
    def _synthesize_live_response(self, user_query: str, live_web_sources: List[Dict[str, str]], retrieved_contexts: List[Dict[str, Any]]) -> str:
        ...
        if ("ผ้าอ้อม" in q_lower or "ติดเตียง" in q_lower or "adl" in q_lower) and ("ประหยัด" in q_lower or "เตียง" in q_lower or "รถเข็น" in q_lower):
            return (
                ...
                "• **เกณฑ์คุณสมบัติ**: ผู้ป่วยมีคะแนน ADL ≤ 11 (เคสนี้ ADL 4/20 เข้าเกณฑ์ติดเตียงระดับรุนแรง) หรือมีภาวะกลั้นขับถ่ายไม่ได้\n"
                ...
                "• **เอกสารที่ต้องใช้**: สำเนาบัตรประชาชนผู้ป่วยและผู้ดูแล, สำเนาทะเบียนบ้าน, และใบรับรองแพทย์/แบบประเมิน ADL จาก รพ.สต. หรือ รพ. ที่รักษา\n"
